...
function loadWelcome(){
    ...
    if (Js.Center.Common.userData != null) {
        ...
        */
        ...
        getNoticeMsgs();
        var noticeData = Js.Center.Common.getNoticeRecord();
        javaStr +='<dl><dt><span>公告信息</span><span style="padding-left:300px"><a href="#" onClick="menuClick(\'s50\',\' 公告信息\',\'js.loadjs.loadbusinessnoticemoreinfo\')">更多</a></span></dt>' ;
		javaStr +=  '   <dd class="matter"><table>' ;
        if(noticeData != null && noticeData.length > 0){
        	for(var i = 0; i < noticeData.length ; i++){
    			javaStr += "<tr width='380px'><td style='width:80px;'>" + noticeData[i].data.datcreatetime + "</td>";
        		if(noticeData[i].data.vc2filepath!=null){
        			javaStr += "<td style='width:250px;'><a href='#' onClick='noTiceDowmload("+i+")' style='width:250px; display:block; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;' title='" + noticeData[i].data.vc2bultitle + "'>" + noticeData[i].data.vc2bultitle + "</a></td>";
        		}else{
        			javaStr += noticeData[i].data.vc2bulletin;
        		}
        		if(noticeData[i].data.vc2filepath){
        			javaStr += "<td style='padding-left:10px; width:40px'><img id='imgLogo' src='jspack/product/common/Images/icons/arrow.gif' onClick='exportData(\"URL/business/noitce/noticequery.ashx\",\"filepath=" + noticeData[i].data.vc2filepath + "&flag=download\")' style='cursor:pointer'/></td></tr>";
        		} else {
        			javaStr += "<td style='padding-left:10px; width:40px'></td></tr>";
        		}
         ...
         * 渠道平台，行业平台首页 提示：
         * 尊敬的客户：
         *     您好，因平台升级给您带来的不便敬请谅解！升级后客户组上传白名单调整如下：
         * 使用客户组方式上传白名单时，为不影响发送功能，系统即时置为全部生效，而实 际生效时间约为24小时。
         * 如有未明之处，烦请联系商务经理或技术支持，谢谢！
         */
        /*
        javaStr = javaStr + '<dl>';
        javaStr = javaStr + '	<dt><span>首页提示</span></dt>';
        javaStr = javaStr + '    <dd class="matter">尊敬的客户：</dd>';
        javaStr = javaStr + '    <dd class="matter">    您好，因平台升级给您带来的不便敬请谅解！升级后客户组上传白名单调整如下：</dd>';
        javaStr = javaStr + '    <dd class="matter">使用客户组方式上传白名单时，为不影响发送功能，系统即时置为全部生效，而实 际生效时间约为24</dd>';
        javaStr = javaStr + '    <dd class="matter">小时。如有未明之处，烦请联系商务经理或技术支持，谢谢！</dd>';
        javaStr = javaStr + '</dl>';
        */
...
function doExit(){
    Ext.Ajax.request({
        url: "URL/userLogin.ashx?flag=exit",
        method: "POST",
...
function loadCurrentUserinfo(){
    ...
    if (Js.Center.Common.getUserRecord() != null) {
        ...
        USERIOGRAPH = Js.Center.Common.userMobile + " " + Js.Center.Common.userName;
        USERIOGRAPHSIZE = USERIOGRAPH.length;
    }
    else {
...
Ext.onReady(function(){
    ...
    //    });
    //}, 200);
    Ext.BLANK_IMAGE_URL = 'jspack/product/common/Images/default/s.gif';
 ...
 new Ext.Toolbar.TextItem({
            ...
            items: [{
                ...
                border: false,
                items: [new Ext.Toolbar.TextItem({
                	text: '<b style=\"display:none\">服务器时间：</b><font id="systemTime" style="font-weight: bold"></font>'
                }), '->', new Ext.Toolbar.TextItem({
                    text: '<font class="x-btn-text-bottomrighticon">版权所有©北京无线天利移动信息技术股份有限公司</font>'
                })]
            }]
        }]
    });
...
//这个是soundManager提供的swf文件所在的文件夹
    soundManager.url = 'JS/product/common/commoncontrol/extsound/swf';
    ...
    soundManager.onload = function() {
        ...
        soundManager.createSound({
            id: 'alertSound',
            url: 'JS/product/common/commoncontrol/extsound/mp3/alarm_3.mp3',
            //autoLoad: true,//自动加载
            //multiShot: false,//true 在同一时刻只能有一个频段的声音
            autoPlay: false //自动播放 这个是系统的背景音
            //volume: 100
        });
...
/**
 * ajax 请求服务器时间并显示在页面左下角
 */
function getSystemTime(){
    Ext.Ajax.request({
        url: "URL/System/SystemQuery.ashx?flag=getsystemtime",
        method: "POST",
        success: function(form, action){
            var obj = Ext.util.JSON.decode(form.responseText);
            var status = obj.success;
            if(status == true && obj.time != 'undefined') {
            	document.getElementById("systemTime").innerHTML = obj.time;
            	loadSystemTime();
            }
        }
    });
...
function getNoticeMsgs(){
	Js.Center.Common.noticeData = doSynRequest("URL/business/noitce/noticequery.ashx?flag=getnotice&numuserid&vc2bultitle&numseqid&datstart&datend");
    //Js.Center.Common.selectcheckbycurrentuserData = doSynRequest(getUserURL + "?flag=selectcheckbycurrentuser");
    Js.Center.Common.getNoticeRecord = function(){
        if (Js.Center.Common.noticeData != null) {
            var reader = new Ext.data.JsonReader({
                totalProperty: 'totalProperty',
                root: 'data',
                fields: ["numseqid",// 公告编号
                         "vc2bultitle",//公告标题
            			 "vc2bulletin",//内容
            			 "datcreatetime",//创建日期
            			 "numuserid",//创建人编号
            ...
            if (Js.Center.Common.noticeData.info == null) {
                if (Js.Center.Common.noticeData.totalCount != 0) {
                    var responses = reader.readRecords(Js.Center.Common.noticeData);
                    var userRecord = responses.records;
                    return userRecord;
                }
            }
        }
...
Ext.namespace('Js.Center.Main.Notice.noticemsg.infoDetail');
Js.Center.Main.Notice.noticemsg.infoDetail.func = function() {	
	if (Ext.get("Js.Center.Main.Notice.noticemsg.infoDetail.window") == null) {
	    //=============================================================产品下拉列表数据定义
	    var noticemsgInfoFormPanel = new Ext.form.FormPanel({
	        ...
	        items: [{
                ...
                html:"<div id='Js.Center.Main.Notice.noticemsg.infoDetail.downloadinfo'></div>"
	        }]
	    });
	    ...
	    this.window = new WXTL.Widgets.CommonWindows.Window({
	        ...
	        buttons: [{
				...
				qtip: "确定",
				handler: function() {
					Js.Center.Main.Notice.noticemsg.infoDetail.window.close();
					mainForm.reset();
				}
			}]
	    });
...
function noTiceDowmload(id){
	var row = Js.Center.Common.noticeData.data[id];
	var updateRow= {data:row};
	Js.Center.Main.Notice.noticemsg.infoDetail.func();
	Js.Center.Main.Notice.noticemsg.infoDetail.window.updateRecord = updateRow;
	Js.Center.Main.Notice.noticemsg.infoDetail.window.show();
	if(row.vc2filepath!=null&&row.vc2filepath!=""){
		Ext.get("Js.Center.Main.Notice.noticemsg.infoDetail.downloadinfo").dom.innerHTML = "<a href='#' onClick='exportData(\"URL/business/noitce/noticequery.ashx\",\"filepath=" + row.vc2filepath+ "&flag=download\")' ><center>下载附件</center> </a>";
	}
};
