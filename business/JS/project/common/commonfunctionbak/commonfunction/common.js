...
/************************************************
*功能描述：上传控件文件验证，大小、类型等
*filePath：待验证的文件路径
*permitFileTypeArr:自定义允许的文件类型,Array
*permitFileTypeDesc：自定义文件类型验证描述
*************************************************/
function checkFileWithTypeArr(filePath, permitFileTypeArr, permitFileTypeDesc) {
	if (permitFileTypeArr == null) {
		...
		if (!checkFileTypeWithTypeArr(filePath, permitFileTypeArr)) {
			return '文件类型应为' + permitFileTypeDesc;
		} else {
			if (getFileSize(filePath) != '' && getFileSize(filePath) > fileSize) {
				return '文件大小应小于' + fileSizeDesc;
			} else {
 ...
 * @param {}  urlStr
 * @param {}  paramsStr 为字符串键值对形式“key=value&key2=value2”
...
function checkMMS(jsonMMSContent){
    ...
    if(mmsConfigInfo == null){
		...
		if(numMMSSpace > mmsConfigInfo.mmstotalsize){
			returnResault = false;
            Ext.Msg.alert("温馨提示", "对不起，彩信大小不能超过" + mmsConfigInfo.mmstotalsize/1024 + "K!");
            return returnResault;
		}
	}
    return returnResault;
...
function checkMMSFrameImageType(isNeedImage, vc2ImageUrl){
    if (!isNeedImage) {
        if (!checkImageFileType(vc2ImageUrl)) {
            Ext.Msg.alert("温馨提示", "对不起,帧图片只能上传"+MMSIMAGEFILETYPEDESC+"文件类型");
            return false;
        }//不是正确的图片类型
    }
    return true;
...
function checkMMSFrameMusicType(isNeedMusic, vc2MusicUrl){
    if (!isNeedMusic) {
        if (!checkMusicFileType(vc2MusicUrl)) {
            Ext.Msg.alert("温馨提示", "对不起,帧背景音乐只能上传"+MMSMUSICFILETYPEDESC+"文件类型");
            return false;
        }//不是正确的音乐类型
    }
    return true;
