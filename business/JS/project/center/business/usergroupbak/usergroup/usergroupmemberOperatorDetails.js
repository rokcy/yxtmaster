       ...
       Js.Center.Business.UserGroupMemberOperatorDetails.Infostore = new WXTL.Widgets.CommonData.GroupingStore({
        ...
        var cm = new Ext.grid.ColumnModel([{
            ...
            renderer: function(value, meta, record, rowIndex, colIndex, store){
                if(value > 0){                	if(record.data.numstate == "处理成功"){                		return "<a href='#' onclick='exportData(\"" + Js.Center.Business.UserGroupMemberURL + "\",\"id=" + record.data.numlogid + "&flag=selectdesc&successtype=-1\")'>"+ value +"</a>";                	}else{                		return value;                	}
                }
                else{
                    return value;
                }
            ...
            renderer: function(value, meta, record, rowIndex, colIndex, store){
                if(value > 0){
                    return "<a href='#' onclick='exportData(\"" + Js.Center.Business.UserGroupMemberURL + "\",\"id=" + record.data.numlogid + "&flag=selectdesc&successtype=0\")'>"+value+"</a>";
                }
                else{
                    return value;
                }
